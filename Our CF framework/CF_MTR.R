...
CF_MTR_Func <- function(curr_wd,xun,pe,vgval,voval,mi, u){
...
### To apply for one KO case, say "aceA" gene
CF_MTR_Func(curr_wd,"aceA",0.75,8.5,14.5,3,1)

### To apply for many genes (one by one)
KO_vec <- c("aceA", "fnr","nhaR")
tu <- length(KO_vec)

for(i in 1:tu){
  CF_MTR_Func(curr_wd,KO_vec[i],0.75,8.5,14.5,3,i)
}
