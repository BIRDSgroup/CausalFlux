...
CF_MTR_Version_2 <- function(curr_wd,xun,pe,vgval,voval,mi, u){
  ...
  # Integration_initial(xu_n[j],gene_subsys, grn_SL, grn_PL,gge, curr_wd, count,FVA_up)
  ...
  Integration_initial <- function(KO_gene, GS,stra,para, g ,wds, ct,fu){
    if(KO_gene == "WT"){
      setwd(wds)
      matlabr::run_matlab_script("CF_V2_Init_1_ID.m", display = TRUE, verbose = TRUE)
      
      # FVA_iplus1 <- read_xlsx(paste0(curr_wd,"/FVA_to_check_P2.xlsx"), col_names = FALSE)
      # write.csv(FVA_iplus1, paste0("FVA_iterations_P2_",ct,".csv"))
      
      FBA_iplus1 <- read_csv("FBA_to_check_P2.csv", col_names = FALSE)
      write.csv(FBA_iplus1, paste0("FBA_iterations_P2_",ct,".csv"))
      ...
      for(i in 1:length(tg_g)){
        ...
        qtxt_1 <- paste0("cpquery(para, ","(",paste(noquote(tg_g[i])),"==",paste("1"),")",",",paste(noquote(KO_gene)),"==",paste("0"),")")
      ...
      writexl::write_xlsx(int2_res$CP_Final, paste0(wds,"/CP_round_P2_i.xlsx"),col_names = TRUE)
      
      cp <- read_xlsx(paste0(wds,"/CP_round_P2_i.xlsx"), col_names = FALSE)
      write.csv(cp, paste0("CP_iterations_P2_",ct,".csv"))
      
      
      # step - 2d - part 1
      setwd(wds)
      matlabr::run_matlab_script("CF_V2_Iteration_2d_ID.m", display = TRUE, verbose = TRUE)
      
      gpr_eval_round_i <- read_xlsx(paste0(wds,"/GPR_eval_round_P2_i.xlsx"), col_names = FALSE)
      ...
      writexl::write_xlsx(new_upd_fva,paste0(wds,"/Updated_FVA_round_P2_i.xlsx"))
      
      mup <- read_xlsx(paste0(wds,"/Updated_FVA_round_P2_i.xlsx"), col_names = FALSE)
      write.csv(mup, paste0("Updated_FVA_round_incorp_P2_",ct,".csv"))
      
      
      # step - 2a - run FVA with 0 objective
      setwd(wds)
      matlabr::run_matlab_script("CF_V2_Iteration_2a_ID.m", display = TRUE, verbose = TRUE)
  ...
  load("D:/work/Integrated_network_model/Ecoli_intg_ntwk/metabolic_aspect/Auto_RUN/Causal_Surgery/Ishii_dataset/GRN_REQ_WO_BIGG/train_data_bn.RData")
  ...
  FVA_round_0 <- readxl::read_xlsx(paste0(curr_wd,"/FVA_1b_obj_0_P2.xlsx"), col_names = FALSE)
  colnames(FVA_round_0) <- c("Reactions", "Minimum_flux", "Maximum_flux")
  FVA_round_0 <- as.data.frame(FVA_round_0)
  
  count <- 1
  
  # FVA_bef <- read_xlsx(paste0(curr_wd,"/FVA_to_check_P2.xlsx"), col_names = FALSE)
  # write.csv(FVA_bef, paste0("FVA_iterations_P2_",count-1,".csv"))
  
  FBA_bef <- read_csv("FBA_to_check_P2.csv", col_names = FALSE)
  write.csv(FBA_bef, paste0("FBA_iterations_P2_",count-1,".csv"))
  ...
  setwd("D:/work/Integrated_network_model/Ecoli_intg_ntwk/metabolic_aspect/Auto_RUN/Causal_Surgery/Ishii_dataset/MM_REQ/")
  # Our IAF1260 data
  ...
  #BG_dat <- readRDS("Bigg_ids_gene_names.RDS")
  ...
  for(j in 1:length(xu_n)){
    ...
    repeat{
      ...
      writexl::write_xlsx(op_intg_2$CP_Final, paste0(curr_wd,"/CP_round_P2_i.xlsx"),col_names = TRUE)
      
      cp <- read_xlsx(paste0(curr_wd,"/CP_round_P2_i.xlsx"), col_names = FALSE)
      write.csv(cp, paste0("CP_iterations_P2_",count+1,".csv"))
      ...
      gpr_eval_round_i <- read_xlsx(paste0(curr_wd,"/GPR_eval_round_P2_i.xlsx"), col_names = FALSE)
      gpr_eval_round_i <- as.data.frame(gpr_eval_round_i)
      colnames(gpr_eval_round_i) <- "gpr_eval"
      ...
      writexl::write_xlsx(new_upd_fva,paste0(curr_wd,"/Updated_FVA_round_P2_i.xlsx"))
      ...
      # FVA_iplus1 <- read_xlsx(paste0(curr_wd,"/FVA_to_check_P2.xlsx"), col_names = FALSE)
      # write.csv(FVA_iplus1, paste0("FVA_iterations_P2_",count+1,".csv"))
      # 
      FBA_iplus1 <- read_csv("FBA_to_check_P2.csv", col_names = FALSE)
      write.csv(FBA_iplus1, paste0("FBA_iterations_P2_",count+1,".csv"))
    ...
    #cc <- read.csv("Counts_done.csv")
    #cc_ <- cc[[1]]
    # vv <- c()
    # for(i in 0:(count)){
    #   vv <- c(vv,paste0("FVA_iterations_P2_",i,".csv"))
    # }
    
    bb <- c()
    for(i in 0:(count)){
      bb <- c(bb,paste0("FBA_iterations_P2_",i,".csv"))
    }
    
    pp <- c()
    for(i in 1:(count)){
      pp <- c(pp,paste0("CP_iterations_P2_",i,".csv"))
    }
    ...
    my_files <- c("TR_init_0.csv","Initial_Max_Turnover.csv","Iteration_Turnover.csv","FVA_1b_obj_0_P2.xlsx","CP_round_P2_i.xlsx","FBA_to_check_P2.csv","GPR_eval_round_P2_i.xlsx","Updated_FVA_round_P2_i.xlsx",ll,bb,pp,rr)
  ...
  library(ff)
  dir.create(paste0(curr_wd,"/Case_P2_",u))
  ...
  for(i in 1:length(m)){
    ...
    path2 <- paste0(to,"/Case_P2_",u,"/",m[i])
...
curr_wd <- c("D:/work/Integrated_network_model/Ecoli_intg_ntwk/metabolic_aspect/Auto_RUN/Causal_Surgery/Parallel_Runs/CF_MTR/")


setwd("D:/work/Integrated_network_model/Ecoli_intg_ntwk/metabolic_aspect/Auto_RUN/Causal_Surgery/Parallel_Runs/")
library(readr)
RD_check <- read_csv("Train_50_2.csv")

tu <- nrow(RD_check) 


for(i in 1:tu){
  CF_MTR_Version_2(curr_wd,RD_check[[1]][i],0.75,8.5,14.5,3,i)
}


s <- c()
for(i in 1:tu){
  s <- c(s,paste0("Case_P2_",i))
}

dir.create(paste0(curr_wd,"/CF_MTR_Train_50_2"))
from <- curr_wd            #Current path of your folder
to   <- curr_wd            #Path you want to move it.
...
for(i in 1:tu){
  path1 <- paste0(from,"/",m[i])
  path2 <- paste0(to,"/CF_MTR_Train_50_2/",m[i])
  file.rename(path1,path2)
}
