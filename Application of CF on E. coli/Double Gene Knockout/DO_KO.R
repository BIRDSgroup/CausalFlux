...
Double_KO_CF_S <- function(curr_wd,xun,xum,pe,vgval,voval,mi, u){
  ...
  Integration_part_1 <- function(df_1, bn_obj, GS, KO1, KO2){
    ...
    KO_t <- c(KO1,KO2)
      ...
      bn_f <- which(bn_arcs$from %in% KO_t)
  ...
  Integration_initial <- function(KO_gene1,KO_gene2, GS,stra,para, g ,wds, ct,fu){

      x <- GRN_CS(KO_gene1,stra,para, g,c())
      xcspl1 <- x$CS_PL
      
      x <- GRN_CS(KO_gene2,x$CS_SL,x$CS_PL, g,c())
      xcspl <- x$CS_PL
      
      ga <- data.frame(stra$arcs)
      
      tg_g <- c()
      KO_gene_t <- c(KO_gene1, KO_gene2)
      x_l <- which(ga$from %in% KO_gene_t)
      if(length(x_l) != 0){
        tg_g <- ga$to[x_l]
        tg_g[length(tg_g)+1] <- KO_gene1
        tg_g[length(tg_g)+1] <- KO_gene2
      }else{
        tg_g <- KO_gene_t
      }
      ...
      for(i in 1:length(tg_g)){
        ...
        qtxt_1 <- paste0("cpquery(xcspl, ","(",paste(noquote(tg_g[i])),"==",paste("1"),")",",","e, method =",paste("'lw'"),")")
        x <- eval(parse(text = qtxt_1))
        all_tgs_eval_bde <- c(all_tgs_eval_bde,x)
      }
  ...
  Integration_part_2 <- function(op_lis_int_1, KO_gene1,KO_gene2, GS,para, strat){
      ...
      CP_cal_func <- function(op_lis, e){
        ...
        tg_uni[length(tg_uni)+1] <- KO_gene1
        tg_uni[length(tg_uni)+1] <- KO_gene2
  ...
  to_check_sink_rxn_iter <- function(fva_pre, fva_1, fva_2,r_begin, r_end,p){
    ...
    sink_FVA_round_2 <- fva_2[[2]][r_begin:r_end] # change the sink reaction indices -- Corresponds to Toy_Model
    
    sink_FVA_round_1 <- fva_1[[2]][r_begin:r_end] 
    
    sink_FVA_round_pre <- fva_pre[[2]][r_begin:r_end] 
  ...
  for(j in 1:length(xu_n)){
    ...
    ifva <- Integration_initial(xu_n[j],xu_m[j] ,gene_subsys, grn_SL, grn_PL,gge, curr_wd,count, FVA_up)
    ...
    repeat{
      ...
      # step - 2c - part 1
      op_intg_1 <- Integration_part_1(NEW_MGR, grn_SL, gene_subsys, xu_n[j],xu_m[j])
      # step - 2c - part 2
      ...
      #NEW_MGR_prime <- new_met_gene_reg_data
        ...
        x <- GRN_CS(xu_m[j],x$CS_SL, x$CS_PL, gge, op_intg_1)
        xcspl <- x$CS_PL
        
        op_intg_2 <- Integration_part_2(op_intg_1,xu_n[j],xu_m[j], gene_subsys,xcspl, grn_SL)
    ...
    my_files <- c("CP_round_P1_i.xlsx","FBA_to_check_P1.csv","FVA_to_check_P1.xlsx","GPR_eval_round_P1_i.xlsx","Updated_FVA_round_P1_i.xlsx",vv,bb,pp,uu,mm)
    curr_wd_ <- paste0(curr_wd,"/")
    ko_dir <- paste0(curr_wd_,"DOKO/")
    
    dir.create(paste0(curr_wd_,"DOKO/"))
  ...
  library(ff)
  dir.create(paste0(curr_wd,"/Case_DKO_",u))
  ...
  for(i in 1:length(m)){
    ...
    path2 <- paste0(to,"/Case_DKO_",u,"/",m[i])
...
Double_KO_CF_S(curr_wd,xun = "hemF", xum = "hemN",pe = 0,vgval = 8.5,voval = 14.5,mi = 3,u = 2)
